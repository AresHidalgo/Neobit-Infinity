...
import { Package, Calendar, DollarSign, Truck, ShoppingBag } from 'lucide-react';
    ...
    <Link to={routesConfig.orders.detail(order.id)}>
      <BrutalCard className="hover:shadow-brutal-lg transition-all cursor-pointer p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-heading text-2xl uppercase">Pedido #{order.id.split('-')[1]}</h3>
        ...
        <div className="space-y-3 border-t-2 border-gray-100 pt-4">
          ...
          {order.shippedAt && (
            <div className="flex items-center gap-3 font-mono text-xs text-gray-500">
              ...
              <span>Enviado el {format(new Date(order.shippedAt), 'dd MMM yyyy')}</span>
            </div>
          )}
        </div>
      </BrutalCard>
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <BrutalCard className="py-12 text-center">
          <p className="font-mono text-red-600 font-bold uppercase">Error al cargar pedidos</p>
        </BrutalCard>
      </div>
    );
  }
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="font-heading text-4xl uppercase border-b-4 border-black inline-block">Mis Pedidos</h1>
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent className="border-3 border-black rounded-none">
              <SelectItem value="all" className="font-mono">Todos</SelectItem>
              <SelectItem value="pending" className="font-mono">Pendiente</SelectItem>
              <SelectItem value="confirmed" className="font-mono">Confirmado</SelectItem>
              <SelectItem value="processing" className="font-mono">Procesando</SelectItem>
              <SelectItem value="shipped" className="font-mono">Enviado</SelectItem>
              <SelectItem value="delivered" className="font-mono">Entregado</SelectItem>
              <SelectItem value="cancelled" className="font-mono">Cancelado</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <BrutalCard className="py-20 text-center bg-gray-50">
          ...
          <h2 className="font-heading text-3xl uppercase mb-4">No se encontraron pedidos</h2>
          <p className="font-mono text-lg text-gray-600 mb-8">
            {statusFilter !== 'all' ? 'No hay pedidos con este estado' : 'Aún no has realizado ningún pedido'}
          </p>
          {statusFilter === 'all' && (
