...
import { Trophy, Package, Heart, Settings, CreditCard, LogOut, Star, Zap } from 'lucide-react';
  ...
  const renderContent = () => {
    switch (activeTab) {
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <BrutalCard variant="neon" className="relative overflow-hidden group">
                ...
                <h3 className="font-mono font-bold uppercase text-sm mb-2">Level</h3>
                ...
                <p className="font-mono text-xs mt-2 font-bold">{stats.xp} / {stats.nextLevelXp} XP</p>
              </BrutalCard>

              <BrutalCard className="bg-neon-pink text-white border-white shadow-[4px_4px_0px_0px_#ffffff]">
                <h3 className="font-mono font-bold uppercase text-sm mb-2">Rank</h3>
                <p className="font-heading text-4xl leading-tight">{stats.rank}</p>
              ...
              <BrutalCard variant="dark">
                <h3 className="font-mono font-bold uppercase text-sm mb-2 text-neon-green">Balance</h3>
                <p className="font-heading text-5xl text-white">$1,337.00</p>
                <BrutalButton size="sm" variant="outline" className="mt-4 w-full border-white text-white hover:bg-white hover:text-black">
                  Top Up
                </BrutalButton>
              </BrutalCard>
            </div>
            ...
            {/* Badges Section */}
            <BrutalCard>
              <h3 className="font-heading text-2xl uppercase mb-6 border-b-4 border-black inline-block">Badges Collection</h3>
              <div className="flex flex-wrap gap-4">
                {stats.badges.map((badge, i) => (
                  <BrutalBadge key={i} variant="neon" size="md" className="text-lg py-2 px-4 border-3">
                    {badge}
                  </BrutalBadge>
                ))}
                <BrutalBadge variant="outline" size="md" className="text-lg py-2 px-4 border-3 border-dashed opacity-50">
                  ??? Locked
                </BrutalBadge>
              </div>
            </BrutalCard>
            ...
            <BrutalCard>
              <h3 className="font-heading text-2xl uppercase mb-6 border-b-4 border-black inline-block">Recent Activity</h3>
              <div className="space-y-4 font-mono">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex items-center justify-between border-b-2 border-gray-200 pb-4 last:border-0">
                    <div>
                      <p className="font-bold">Order #882{i}</p>
                      <p className="text-sm text-gray-500">Purchased Cyber Deck v2</p>
                    </div>
      ...
      case 'settings':
        return (
          <BrutalCard>
            <h3 className="font-heading text-2xl uppercase mb-8">Account Settings</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <BrutalInput label="Display Name" defaultValue={user?.firstName} />
              <BrutalInput label="Email" defaultValue={user?.email} disabled />
              <BrutalInput label="Cyber Handle" defaultValue="@neobit_user" />
              <BrutalInput label="Location" defaultValue="Neo Tokyo, Sector 7" />
            </div>
        ...
        return (
          <div className="flex flex-col items-center justify-center h-64 text-center">
            ...
            <p className="font-mono text-lg max-w-md">
              This module is currently being compiled by our nanobots. Check back later.
            </p>
          </div>
        );
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          <BrutalCard className="text-center">
            ...
            <h2 className="font-heading text-2xl uppercase">{user?.firstName} {user?.lastName}</h2>
            <p className="font-mono text-sm text-gray-500 mt-1">Member since 2077</p>
          </BrutalCard>
